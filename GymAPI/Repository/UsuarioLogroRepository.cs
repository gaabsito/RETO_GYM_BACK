...
namespace GymAPI.Repositories
{
    public class UsuarioLogroRepository : IUsuarioLogroRepository
    {
        ...
        // Implementación de GetByUsuarioAndLogroIdAsync
        public async Task<UsuarioLogro?> GetByUsuarioAndLogroIdAsync(int usuarioId, int logroId)
        {
            UsuarioLogro? usuarioLogro = null;
            ...
            using (var connection = new SqlConnection(_connectionString))
            {
                ...
                string query = @"SELECT ul.UsuarioLogroID, ul.UsuarioID, ul.LogroID, 
                               ...
                               FROM UsuarioLogros ul
                               JOIN Logros l ON ul.LogroID = l.LogroID
                               WHERE ul.UsuarioID = @UsuarioId AND ul.LogroID = @LogroId";
        ...
        public async Task<int> AddAsync(UsuarioLogro usuarioLogro)
        {
            using (var connection = new SqlConnection(_connectionString))
            {
                await connection.OpenAsync();
                string query = @"INSERT INTO UsuarioLogros 
                               (UsuarioID, LogroID, FechaDesbloqueo, ProgresoActual, Desbloqueado)
                               VALUES (@UsuarioId, @LogroId, @FechaDesbloqueo, @ProgresoActual, @Desbloqueado);
                               SELECT SCOPE_IDENTITY();";

                using (var command = new SqlCommand(query, connection))
                {
                    command.Parameters.AddWithValue("@UsuarioId", usuarioLogro.UsuarioID);
                    command.Parameters.AddWithValue("@LogroId", usuarioLogro.LogroID);
                    
                    // Si FechaDesbloqueo es el valor default (y logro no está desbloqueado), usar NULL
                    if (usuarioLogro.Desbloqueado)
                    {
                        command.Parameters.AddWithValue("@FechaDesbloqueo", usuarioLogro.FechaDesbloqueo);
                    }
                    else
                    {
                        command.Parameters.AddWithValue("@FechaDesbloqueo", DBNull.Value);
                    }
                    
                    command.Parameters.AddWithValue("@ProgresoActual", usuarioLogro.ProgresoActual);
                    command.Parameters.AddWithValue("@Desbloqueado", usuarioLogro.Desbloqueado);
        ...
        public async Task UpdateAsync(UsuarioLogro usuarioLogro)
        {
            using (var connection = new SqlConnection(_connectionString))
            {
                await connection.OpenAsync();
                string query = @"UPDATE UsuarioLogros 
                               SET ProgresoActual = @ProgresoActual,
                                   Desbloqueado = @Desbloqueado,
                                   FechaDesbloqueo = @FechaDesbloqueo
                               WHERE UsuarioLogroID = @Id";

                using (var command = new SqlCommand(query, connection))
                {
                    command.Parameters.AddWithValue("@Id", usuarioLogro.UsuarioLogroID);
        ...
        public async Task DeleteAsync(int id)
        {
            ...
            {
                await connection.OpenAsync();
                string query = @"DELETE FROM UsuarioLogros WHERE UsuarioLogroID = @Id";
